...
public class Dropout {
	...
	public void train(int epochs, double[][] train_X, int[][] train_Y, boolean dropout, double p_dropout, double lr) {
		...
		for (int epoch = 0; epoch < epochs; epoch++) {
			...
			for (int n = 0; n < N; n++) {
				...
				layer_inputs = new ArrayList<>(n_layers + 1); // +1 for logistic
																// layer
	...
	public void pretest(double p_dropout) {
		for (int i = 0; i < n_layers; i++) {
			...
			if (i == 0)
				...
				in = hidden_layer_sizes[i];

			if (i == n_layers - 1)
				out = n_out;
			else
				out = hidden_layer_sizes[i + 1];
	...
	private static void test_dropout() {
		...
		double[][] train_X = { { 0., 0. }, { 0., 1. }, { 1., 0. }, { 1., 1. }, };

		int[][] train_Y = { { 0, 1 }, { 1, 0 }, { 1, 0 }, { 0, 1 }, };
