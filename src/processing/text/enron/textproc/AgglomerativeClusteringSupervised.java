...
public class AgglomerativeClusteringSupervised extends Clustering {
	...
	public AgglomerativeClusteringSupervised(ArrayList<Email> train_emails, ArrayList<Email> test_emails, double threshold,
	...
	public AgglomerativeClusteringSupervised(ArrayList<Email> test_emails, double threshold,
			String classifierModelFile, ArrayList<String> features,String DATASET, String distanceCalculation, long maxTimeSpan, RelatednessCalculator wordnet, Table<Integer, Integer, Double> score_AlreadyCalc, String experimentComment) {
		clusters = new ArrayList<>();	
		for (Email e : test_emails) {
			Cluster c=new Cluster();
			c.insertEmail(e);
		...
		this.wekamodel.loadClassifierFromFile(classifierModelFile);

		this.score_AlreadyCalc=score_AlreadyCalc;
	}
	
	public AgglomerativeClusteringSupervised(ArrayList<Email> test_emails, double threshold,
			Classifier classifier, ArrayList<String> features,String DATASET, String distanceCalculation, long maxTimeSpan, RelatednessCalculator wordnet, Table<Integer, Integer, Double> score_AlreadyCalc, String experimentComment) {
		clusters = new ArrayList<>();	
		for (Email e : test_emails) {
			Cluster c=new Cluster();
			c.insertEmail(e);
	...
	private double calcClustersSimilarity(Cluster c1, Cluster c2){
		...
		if(distanceCalculation.equals("centroid")){			
			...
			similarity=wekamodel.classifyEmailPair(centroid1, centroid2, test_emails)[1];		
		}
			...
			for(Email e1 : c1.getEmails()){
				for(Email e2 : c2.getEmails()){
					...
					if(sim==null){
						sim=wekamodel.classifyEmailPair(e1, e2, test_emails)[1];
