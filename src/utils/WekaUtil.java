...
public class WekaUtil {
	...
	public static Instances creteWekaEmailRandomPairsInstances(ArrayList<Email> emails, ArrayList<String> features, int numInstSameThread,int numInstDifferentThread, long randomSeed, EmailSimGlobalCloseness esimClosenesses, RelatednessCalculator wordnet){
	...
	public static Instances creteWekaEmailPairsInstances(ArrayList<Email> emails, ArrayList<String> features, int numInstSameThread,int numInstDifferentThread, EmailSimGlobalCloseness esimClosenesses, RelatednessCalculator wordnet){
		ArrayList<Attribute> atts = new ArrayList<>();
		ArrayList<String> nominalAtt = new ArrayList<>();
		nominalAtt.add("different");
		nominalAtt.add("same");
		...
		try{
			...
			for(int i=0;i<emails.size();i++){
				...
				for(int j=0;j<emails.size();j++){
					...
					else if(ei.getThreadid()!=ej.getThreadid() && contInstDifferentThread<numInstDifferentThread){
						//different thread
						double[] vals = new double[wekainst.numAttributes()];
						vals[0]=wekainst.attribute(0).addStringValue(ei.getId()+"_"+ej.getId());
						int f=1;				
	...
	public static Instances creteWekaEmailPairsSingleInstance(Email email1,Email email2, ArrayList<String> features, EmailSimGlobalCloseness esimClosenesses, ArrayList<Email> emails, RelatednessCalculator wordnet){
		ArrayList<Attribute> atts = new ArrayList<>();
		ArrayList<String> nominalAtt = new ArrayList<>();
		nominalAtt.add("same");
		nominalAtt.add("different");
